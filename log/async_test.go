...
func asyncSimpleLogger(wg *sync.WaitGroup) {
	...
	logger, _ := NewAsyncSimpleBufferFileLogger(w, 1024, LEVEL_INFO, time.Second*2, NewAsyncLogRoutine(10))
...
func asyncWebLogger(wg *sync.WaitGroup) {
	...
	logger, _ := NewAsyncSimpleWebBufferFileLogger(w, []byte("async_web"), 1024, LEVEL_INFO, time.Second*2, NewAsyncLogRoutine(10))
